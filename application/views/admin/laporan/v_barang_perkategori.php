                ...
                <td colspan="2" style="width:800px;paddin-left:20px;">
                    <center>
                        <?php foreach ($data->result_array() as $d) {
                            $katego = $d['kategori_nama'];
                            $query = $this->db->query("SELECT kategori_id,kategori_nama FROM tbl_barang JOIN tbl_kategori ON kategori_id=barang_kategori_id WHERE kategori_nama='$katego' GROUP BY kategori_nama");
                            $t = $query->row_array();
                        }
                        $kt = $t['kategori_nama'];
                        echo "<h4>LAPORAN DATA BARANG PERKATEGORI $kt </h4>";
                        ?>
                    </center><br />
                </td>
            </tr>
