...
class Pembelian extends CI_Controller{
	...
	if($this->session->userdata('akses')=='1'){
		$x['data'] = $this->m_pembelian->get_all();
		$this->load->view('admin/v_beli_barang',$x);
	...
	public function get_stok($kobar)
		{
			...
			$barang = array();
			$barang[] = array('id'=>$b['barang_id'], 'nama'=>$b['barang_nama'], 'harpok'=>$b['barang_harpok'], 'harjul'=>$b['barang_harjul'], 'satuan'=>$b['barang_satuan'], 'grosir'=>number_format($b['barang_harjul_grosir']));
	...
	if($this->session->userdata('akses')=='1'){
		...
		redirect('admin/pembelian/tambah_beli');
	}if($this->session->userdata('akses')=='2'){
		$nofak=$this->input->post('nofak');
		$tgl=$this->input->post('tgl');
		$suplier=$this->input->post('suplier');
